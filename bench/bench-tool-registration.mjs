...
const outDir = path.join(rootDir, "bench/.tool-registration-build");
...
async function writeReport({ rootDir, report, markdown }) {
	...
	await writeFile(
		path.join(dir, `tool-registration-${report.generatedAt}.json`),
		`${JSON.stringify(report, null, 2)}\n`,
	);
	await writeFile(path.join(dir, "tool-registration-latest.md"), markdown);
}
