...
import { stringFlag } from "../harness/cli-args.mjs";
import {
	clean,
...
async function writeReport(report, markdown) {
	...
	await writeFile(
		path.join(dir, `dom-adapter-diff-stability-${report.generatedAt}.json`),
		`${JSON.stringify(report, null, 2)}\n`,
	);
	await writeFile(
		path.join(dir, "dom-adapter-diff-stability-latest.md"),
		markdown,
	);
}
