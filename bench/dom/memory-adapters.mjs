...
if (!globalThis.gc) {
	console.error("Run with `node --expose-gc bench/dom/memory-adapters.mjs`.");
	process.exit(2);
}
...
async function writeReport(report, markdown) {
	...
	await writeFile(
		path.join(dir, `dom-adapter-memory-${report.generatedAt}.json`),
		`${JSON.stringify(report, null, 2)}\n`,
	);
	await writeFile(path.join(dir, "dom-adapter-memory-latest.md"), markdown);
}
