...
async function writeCompareReport({ rootDir, kind, report, markdown }) {
	const dir = path.join(rootDir, "bench/results");
	await mkdir(dir, { recursive: true });
	await writeFile(
		path.join(dir, `${kind}-${report.generatedAt}.json`),
		`${JSON.stringify(report, null, 2)}\n`,
	);
	await writeFile(path.join(dir, `${kind}-latest.md`), markdown);
}
