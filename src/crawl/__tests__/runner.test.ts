/**
 * @fileoverview crawl __tests__ runner.test module.
 */
...
describe("runCrawl", () => {
			...
			},
			{
				httpClient: {
					fetchUrl: async (url) => {
						...
						maxByHost.set(
							host,
							Math.max(maxByHost.get(host) ?? 0, activeByHost.get(host) ?? 0),
						);
						maxGlobal = Math.max(
							maxGlobal,
	...
	it("resumes from saved frontier state", async () => {
		...
		const second = await runCrawl(
			...
			{ rootDir, crawlId: "resume", maxPages: 1, maxDepth: 1, resume: true },
			{
				httpClient: {
					fetchUrl: async (url) =>
						html(url.toString(), `<html><main>${url}</main></html>`),
				},
			},
		);
	...
	it("persists crawl metadata and emits compact status progress", async () => {
		...
		expect(progress.some((message) => message.includes("2/2 pages"))).toBe(
			true,
		);
	});
