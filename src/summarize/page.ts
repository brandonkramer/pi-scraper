...
export async function summarizePage(
  ...
  return { input: prepared.input, summary: response.text ?? String(response.data ?? ""), raw: response.raw };
}

async function prepareSummaryInput(options: PageSummaryOptions, deps: ScrapePipelineDeps, signal?: AbortSignal): Promise<{ content: string; input: PageSummaryResult["input"] }> {
  if (options.content?.trim()) {
    return { content: options.content, input: { url: options.url, source: "provided" } };
  }
  if (!options.url) {
    throw new Error("summarizePage requires url or content");
  }
  const scrape = await scrapeUrl(options.url, { ...options, format: options.format ?? "markdown" }, deps, signal);
  return { content: scrape.data.markdown ?? scrape.data.text ?? "", input: { url: options.url, source: "scrape", scrape } };
}
