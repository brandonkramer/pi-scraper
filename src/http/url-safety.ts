...
export function assertSafeUrl(input: string | URL, options: UrlSafetyOptions = {}): SafeUrlResult {
...
function validateHostname(url: URL, options: UrlSafetyOptions): void {
	...
	if (isPrivateOrReservedIp(hostname)) {
		throw new UrlSafetyError(
			"PRIVATE_NETWORK_ADDRESS",
			`Blocked private address: ${hostname}`,
			url.toString(),
		);
	}
}
...
function isPrivateOrReservedIpv6(address: string): boolean {
	...
	// ::/8 reserved/unspecified/IPv4-compatible space.
	// 100::/64 discard-only prefix.
	// fc00::/7 unique local addresses.
	// fe80::/10 link-local addresses.
	// ff00::/8 multicast addresses.
	// 2002::/16 6to4, avoid bypass through embedded IPv4.
