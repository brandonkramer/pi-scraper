...
import { BrowserRenderError, type BrowserRenderer } from "../../browser/playwright.js";
...
describe("scrapeUrl", () => {
  it("scrapes static HTML without browser escalation", async () => {
    ...
    const result = await scrapeUrl(URL, { mode: "auto" }, deps(htmlResponse(articleHtml()), { browserRenderer: browser.renderer }));
  ...
  it("escalates to readable when readable extraction is better", async () => {
    ...
    const result = await scrapeUrl(URL, { mode: "auto" }, deps(htmlResponse("<html><body><h1>Short</h1><p>Thin.</p></body></html>"), {
      readableExtractor: () => ({ ok: true, title: "Readable", textContent: readableText, contentHtml: `<article><p>${readableText}</p></article>` }),
    }));
  ...
  it("recovers useful sparse content from data islands without browser", async () => {
    ...
    const result = await scrapeUrl(URL, { mode: "auto" }, deps(htmlResponse(html), { browserRenderer: browser.renderer }));
  ...
  it("routes PDFs through the PDF extraction boundary", async () => {
    const result = await scrapeUrl(`${URL}.pdf`, { mode: "fast" }, deps({
    ...
    expect(result.data.pdf).toMatchObject({ ok: false, reason: "unsupported" });
  });

  it("returns binary attachment metadata for non-text media", async () => {
    const result = await scrapeUrl(`${URL}.png`, { mode: "fast" }, deps({
      ...baseResponse(`${URL}.png`, "image/png"),
      file: { path: "/tmp/image.bin", contentType: "image/png", downloadedBytes: 12 },
      downloadedBytes: 12,
    }));

    expect(result.data.route).toBe("binary");
    expect(result.data.file).toMatchObject({ path: "/tmp/image.bin", kind: "binary" });
  });

  it("returns structured blocked signals without anti-bot promises", async () => {
    const blocked = htmlResponse(`<html><body><h1>Access denied</h1><p>${"captcha blocked text ".repeat(80)}</p></body></html>`, 403);
    const fingerprint: FingerprintFetchAdapter = { fetch: vi.fn(async () => blocked) };
    const result = await scrapeUrl(URL, { mode: "auto" }, deps(blocked, { fingerprintAdapter: fingerprint }));
  ...
  it("marks failed auto browser attempts distinctly from successful browser renders", async () => {
    const blocked = htmlResponse("<html><body><h1>Access denied</h1><p>captcha</p></body></html>", 403);
    const fingerprint: FingerprintFetchAdapter = { fetch: vi.fn(async () => { throw new Error("fingerprint unavailable"); }) };
    const renderer: BrowserRenderer = {
      fetchRendered: vi.fn(async () => {
        throw new BrowserRenderError({ code: "BROWSER_UNAVAILABLE", phase: "browser", message: "missing", retryable: false, url: URL });
      }),
    };
    const result = await scrapeUrl(URL, { mode: "auto" }, deps(blocked, { fingerprintAdapter: fingerprint, browserRenderer: renderer }));
...
function deps(response: FetchUrlResult, extra: Partial<ScrapePipelineDeps> = {}): ScrapePipelineDeps {
  return { httpClient: { fetchUrl: vi.fn(async () => response) }, ...extra };
}

function htmlResponse(html: string, status = 200): FetchUrlResult {
  return { ...baseResponse(URL, "text/html", status), text: html, body: Buffer.from(html), downloadedBytes: Buffer.byteLength(html) };
}

function baseResponse(url: string, contentType: string, status = 200): FetchUrlResult {
  return { url, finalUrl: url, status, headers: { "content-type": contentType }, contentType, downloadedBytes: 0 };
}
...
function fakeBrowser(): { renderer: BrowserRenderer; calls: () => number } {
  ...
  return {
    calls: () => count,
    renderer: { fetchRendered: vi.fn(async () => { count += 1; return { url: URL, finalUrl: URL, status: 200, html: articleHtml() }; }) },
  };
}
