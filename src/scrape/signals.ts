...
export function analyzeFastResult(
	response: FetchUrlResult,
	extraction: FastPageExtraction,
): ScrapeSignals {
	const visibleTextLength = extraction.text.length;
	const htmlLength =
		extraction.html.length > 0 ? extraction.html.length : (response.text?.length ?? 0);
	const dataIslandTextLength = extraction.dataIslands.reduce(
		(sum, island) => sum + island.text.length,
		0,
	);
	...
	return {
		...
		shouldTryBrowser:
			(spaLikely && sparseDom && dataIslandTextLength < 200) || (blockedLikely && sparseDom),
		reasons,
	};
}
