...
beforeEach(async () => {
	rootDir = await mkdtemp(path.join(tmpdir(), "pi-scraper-tools-"));
...
function smokeParams(name: string): unknown {
	switch (name) {
		...
		case "web_browser":
			// Missing url short-circuits to an input-error shell — no browser launch (Playwright is
			// peer-optional and must not be required in CI smoke).
			return { action: "navigate", sessionId: "smoke" };
		default:
