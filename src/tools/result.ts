import type { OutputFormat, PiToolShell, ResultEnvelope, StructuredError, TimingInfo } from "../types.js";
...
export function toolResult<TData>(options: ResultShellOptions<TData>): PiToolShell<ResultEnvelope<TData>> {
  return {
...
export function errorResult(error: StructuredError, text = error.message): PiToolShell<ResultEnvelope<undefined>> {
  return toolResult({ text, data: undefined, url: error.url, finalUrl: error.finalUrl, status: error.statusCode, error, truncated: false });
}

export function structuredToolError(error: unknown, fallbackCode: string, phase: string, url?: string): StructuredError {
