...
export const webListExtractorsTool = defineWebTool({
  ...
  description: "List deterministic vertical extractors, URL patterns, output schemas, and browser/cloud/LLM capability declarations.",
  parameters: webListExtractorsSchema,
  async execute() {
    const capabilities = listExtractorCapabilities();
    return toolResult({ text: `${capabilities.length} extractor(s): ${capabilities.map((item) => item.name).join(", ")}`, data: capabilities, format: "json" });
  },
  renderCall: (_args, theme) => renderSimpleCall("web_list_extractors", [], theme),
