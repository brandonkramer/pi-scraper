...
export interface VerticalExtractor<T = unknown> {
  ...
  extract(url: URL, match: Record<string, string>, context: VerticalExtractorContext, signal?: AbortSignal): Promise<T>;
}

export function capability(name: string, urlPatterns: string[], schema: unknown, requirements: Partial<Pick<ExtractorCapability, "requiresBrowser" | "requiresLLM" | "requiresCloud">> = {}): ExtractorCapability {
  return {
    name,
    urlPatterns,
