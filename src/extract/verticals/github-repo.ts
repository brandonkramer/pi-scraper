...
export const githubRepoExtractor: VerticalExtractor = {
  capability: capability("github_repo", ["https://github.com/:owner/:repo"], {
    ...
    properties: { fullName: { type: "string" }, url: { type: "string" }, stars: { type: "number" } },
  }),
  match: (url) => {
    if (url.hostname !== "github.com") return undefined;
    const [owner, repo] = url.pathname.split("/").filter(Boolean);
    return owner && repo && !repo.includes(".") ? { owner, repo } : undefined;
  },
  extract: async (_url, match, context, signal) => {
    const repo = await context.fetchJson<GitHubRepoApi>(`https://api.github.com/repos/${match.owner}/${match.repo}`, signal);
    return {
