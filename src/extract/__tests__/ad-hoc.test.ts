...
import { extractAdHoc, MissingExtractInputError, prepareExtractionInput } from "../ad-hoc.ts";
import { MissingModelAdapterError, type ModelAdapter } from "../model.ts";
...
describe("extractAdHoc", () => {
  it("uses provided clean content and injected model boundary", async () => {
    ...
    const result = await extractAdHoc({ content: "# Widget\nGreat product", prompt: "Extract product", schema }, model);
