/** @file Extract verticals pypi module. */
...
export const pypiPackageExtractor: VerticalExtractor = {
	...
	match: (url) => {
		if (url.hostname !== "pypi.org") return;
	...
	extract: async (_url, match, context, signal) => {
		const pkg = await context.fetchJson<PypiPackage>(
			`https://pypi.org/pypi/${encodeURIComponent(match.name)}/json`,
			signal,
		);
		return {
			name: pkg.info.name,
