/** @file Extract verticals ossinsight-trending-repos module. */
...
export const ossInsightTrendingReposExtractor: VerticalExtractor<OssInsightTrendingReposOutput> = {
	capability: capability(
		...
		["https://ossinsight.io/trending", "https://ossinsight.io/trending/:language"],
		{
			type: "object",
	...
	match: (url) => {
		...
		if (!isTrendingPeriod(period)) return;
	...
	extract: async (_url, match, context, signal) => {
		const period = isTrendingPeriod(match.period) ? match.period : "past_24_hours";
		const language = match.language || "All";
		const payload = await context.fetchJson<OssInsightRows<OssInsightTrendingRepoRow>>(
...
function trimTrendingRow(row: OssInsightTrendingRepoRow): OssInsightTrendingRepoRow {
	return {
		...
		...(row.primary_language !== undefined ? { primary_language: row.primary_language } : {}),
