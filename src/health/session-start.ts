...
export async function runSessionStartHealthChecks(
	...
	if (!(await (deps.checkPlaywright?.() ?? checkPlaywrightAvailable()))) {
		warn({
			...
			message:
				"Optional browser mode is unavailable. Install Playwright with `npm install playwright` and `npx playwright install chromium` if browser rendering is needed.",
		});
	}
