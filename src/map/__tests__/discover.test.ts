...
function binaryResult(url: string, body: Buffer): FetchUrlResult {
  return { url, finalUrl: url, status: 200, headers: {}, contentType: "application/gzip", body, text: body.toString("utf8"), downloadedBytes: body.byteLength };
}
...
describe("discoverSiteUrls", () => {
  it("discovers URLs from robots sitemaps, nested sitemaps, and llms.txt without page content", async () => {
    ...
    const bodies = new Map<string, string>([
      ...
      ["https://example.com/sitemap-index.xml", "<sitemapindex><sitemap><loc>https://example.com/posts.xml</loc></sitemap></sitemapindex>"],
      ["https://example.com/posts.xml", "<urlset><url><loc>https://example.com/post-a</loc><lastmod>2026-01-01</lastmod></url></urlset>"],
      ["https://example.com/llms.txt", "# Docs\n[Guide](https://example.com/guide?utm_source=x)"],
    ]);

    const result = await discoverSiteUrls("https://example.com", {}, {
      httpClient: { fetchUrl: async (url) => { const key = url.toString(); calls.push(key); return textResult(key, bodies.get(key) ?? ""); } },
    });

    expect(result.urls.map((entry) => entry.url)).toEqual(["https://example.com/guide", "https://example.com/post-a"]);
  ...
  it("discovers URLs from raw gzipped sitemap bodies", async () => {
    const gzipped = gzipSync("<urlset><url><loc>https://example.com/gz-page?utm_source=x</loc><lastmod>2026-02-01</lastmod></url></urlset>");
    const bodies = new Map<string, FetchUrlResult>([
      ["https://example.com/robots.txt", textResult("https://example.com/robots.txt", "Sitemap: https://example.com/sitemap.xml.gz")],
      ["https://example.com/sitemap.xml", textResult("https://example.com/sitemap.xml", "")],
      ["https://example.com/sitemap.xml.gz", binaryResult("https://example.com/sitemap.xml.gz", gzipped)],
      ["https://example.com/llms.txt", textResult("https://example.com/llms.txt", "")],
    ]);

    const result = await discoverSiteUrls("https://example.com", {}, {
      httpClient: { fetchUrl: async (url) => bodies.get(url.toString()) ?? textResult(url.toString(), "") },
    });
