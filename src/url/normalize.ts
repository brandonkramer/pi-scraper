...
const TRACKING_PARAMS = new Set(COMMON_TRACKING_QUERY_PARAMS.map((param) => param.toLowerCase()));

export function normalizeUrl(input: string | URL, options: NormalizeUrlOptions = {}): string {
	const { stripTrackingParams = true, stripFragment = true, trimTrailingSlash = true } = options;
